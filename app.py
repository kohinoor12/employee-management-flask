...
def addnewemployee():
    ...
    if request.method == "POST":
        ...
        db = get_database()
        db.execute('insert into emp (name, email, designation ,phone ,address) values (?,?,?,?,?)', [name, email, designation ,phone, address])
        db.commit()
        return redirect(url_for('dashboard'))
...
def updateemployee():
    ...
    if request.method == 'POST':
        ...
        address = request.form['address']
        db = get_database()
        db.execute('update emp set name = ?, email =? , designation=? , phone = ? , address = ? where empid = ?', [name, email,designation, phone, address, empid])
        db.commit()
        return redirect(url_for('dashboard'))
